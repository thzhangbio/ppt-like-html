...
const state = { selected: new Set(), history: [], future: [], templateHead: '', dragging: null, resizing: null };
const rolePreset = {
...
async function initTemplateEditor(){
  try{
    ...
    const deck = doc.querySelector('main.deck') || doc.body;
    $('templateMount').innerHTML = deck.outerHTML;
...
function onElementPointerDown(ev){
  ...
  const starts = selected.map(el => ({ el, rect: rectInSlide(el), abs: el.dataset.absolute === '1' }));
  let moved = false;
  const onMove = (e) => {
    ...
    starts.forEach(s => {
      const box = s.rect;
      s.el.style.left = Math.round(box.x + dx) + 'px';
      s.el.style.top = Math.round(box.y + dy) + 'px';
    });
...
function materialize(el){
  ...
  ghost.dataset.ghostFor = ensureId(el);
  ...
  el.dataset.absolute = '1';
  el.dataset.ghostId = ghost.dataset.ghostFor;
...
function rectInSlide(el){
  const slide = el.closest('.slide'); const er = el.getBoundingClientRect(); const sr = slide.getBoundingClientRect();
  return { x: er.left - sr.left, y: er.top - sr.top, w: er.width, h: er.height };
}
function groupRect(items){
  const rects = items.map(rectInSlide); const x = Math.min(...rects.map(r=>r.x)); const y = Math.min(...rects.map(r=>r.y)); const r = Math.max(...rects.map(r=>r.x+r.w)); const b = Math.max(...rects.map(r=>r.y+r.h));
...
function updateSelectionBox(){
  const selected = selectedElements(); const box = $('selectionBox');
  if(!selected.length){ box.classList.add('hidden'); return; }
  const g = groupViewportRect(selected); Object.assign(box.style, { left:g.x+'px', top:g.y+'px', width:g.w+'px', height:g.h+'px' }); box.classList.remove('hidden');
}
...
function exportHtml(){ const clean=deck().cloneNode(true); clean.querySelectorAll('[data-editable]').forEach(n=>{ n.removeAttribute('data-editable'); n.removeAttribute('data-absolute'); n.removeAttribute('data-ghost-id'); n.removeAttribute('data-role'); n.classList.remove('editable-hover','editable-selected'); n.contentEditable='false'; }); clean.querySelectorAll('.editor-ghost').forEach(n=>n.remove()); const html='<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>VIVOID edited deck</title>'+state.templateHead+'</head><body>'+clean.outerHTML+'</body></html>'; download('VIVOID_同风格正式汇报模板套件_edited.html', html, 'text/html'); }
